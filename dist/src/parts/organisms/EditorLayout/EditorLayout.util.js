...
export const generateTools = handleToolSelection => [
...
export const getButtonColumn = (tools, interval) => tools.slice(interval[0], interval[1]).map((tool, index) => {
    const toolboxButton = new ToolboxButton(tool);
    return html `<span class="colum-${index % 2}">${toolboxButton}</span>`;
});
//# sourceMappingURL=EditorLayout.util.js.map